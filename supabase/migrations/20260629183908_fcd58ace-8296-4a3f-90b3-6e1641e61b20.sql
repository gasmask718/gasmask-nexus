...
BEGIN
  ...
  SELECT
    pa.id, pa.product_name, pa.retail_price, pa.wholesaler_id,
    COALESCE(pa.inventory_qty,0),
    COALESCE(sv.daily_velocity, 0),
    CASE WHEN COALESCE(sv.daily_velocity, 0) = 0 THEN 999
         ELSE ROUND(COALESCE(pa.inventory_qty,0)::numeric / sv.daily_velocity, 1) END,
    CASE WHEN COALESCE(sv.daily_velocity, 0) = 0 THEN (now() + interval '999 days')::date
         ELSE (now() + ((COALESCE(pa.inventory_qty,0)::numeric / sv.daily_velocity)::text || ' days')::interval)::date END,
    GREATEST(0, CEIL(COALESCE(sv.daily_velocity,0) * p_days_ahead - COALESCE(pa.inventory_qty,0)))::int,
    CASE
