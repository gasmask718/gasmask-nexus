...
au_raw AS (
  SELECT a.id, a.record_id, a.acted_at, coalesce(a.acted_by, a.actor_user_id) AS actor_id,
         CASE
           ...
           ELSE 'record_updated'
         END AS action,
         public._aa_change_summary(coalesce(a.old_data, a.before), coalesce(a.new_data, a.after), a.changed_fields) AS detail_text,
         row_number() OVER (
           PARTITION BY a.record_id, date_trunc('second', a.acted_at)
           ORDER BY CASE a.table_name WHEN 'stores' THEN 0 ELSE 1 END, a.id
         ) AS rn
  FROM audit_log a
...
base AS (
  ...
  UNION ALL
  SELECT 'au:'||x.id::text, 'correction', x.record_id, x.acted_at, x.actor_id, NULL,
         x.action, NULL, x.detail_text
  FROM au_raw x WHERE x.rn = 1 AND x.detail_text IS NOT NULL
  UNION ALL
  ...
  SELECT 'rt:'||st.id::text, 'route', st.store_id, st.created_at, NULL::uuid, NULL,
         ...
         'Added to '||coalesce(nullif(r.name,''),'route')||coalesce(' ('||r.type||')','')||
