...
-- Update decision check to support new operator decisions
ALTER TABLE public.dynasty_dryrun_feedback DROP CONSTRAINT IF EXISTS dynasty_dryrun_feedback_decision_check;
ALTER TABLE public.dynasty_dryrun_feedback ADD CONSTRAINT dynasty_dryrun_feedback_decision_check
  CHECK (decision = ANY (ARRAY['approve','hold','reject','needs_review','override_winner','skiplist','defer_to_bulk']));

-- Rewrite preview_store_merge_group with:
--   * Phase B intra-batch dedup (merged_from_loser_ids tracked)
--   * needs_review classification (R1..R5)
-- READ-ONLY: no INSERT/UPDATE/DELETE/TRUNCATE statements.
...
-- check_merge_approval helper (read-only, for future executor)
CREATE OR REPLACE FUNCTION public.check_merge_approval(p_group_id integer)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
DECLARE
  ...
  v_recent_approval boolean;
BEGIN
  SELECT (preview_store_merge_group(p_group_id) -> 'merge_summary' ->> 'requires_operator_approval')::boolean
    INTO v_needs_review;

  SELECT EXISTS(
    SELECT 1 FROM dynasty_dryrun_feedback
    WHERE duplicate_group_id = p_group_id
      AND decision = 'approve'
      AND created_at > now() - interval '24 hours'
  ) INTO v_recent_approval;

  RETURN jsonb_build_object(
    'group_id', p_group_id,
    'needs_review', v_needs_review,
    'has_recent_approval', v_recent_approval,
    'safe_to_execute', (NOT v_needs_review) OR v_recent_approval
  );
END;
$$;
