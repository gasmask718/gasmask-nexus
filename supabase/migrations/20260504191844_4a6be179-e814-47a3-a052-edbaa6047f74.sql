...
BEGIN
  ...
  WITH base AS (
    SELECT
      s.id AS sid, s.name AS sname, sm.store_name AS mname,
...
BEGIN
  ...
  SELECT jsonb_agg(jsonb_build_object(
    'store_id', store_id, 'store_name', store_name, 'raw_address', raw_address,
    'phone', phone, 'created_at', created_at, 'is_winner', is_winner
  ) ORDER BY created_at) INTO v_members
  FROM public.detect_store_phone_name_duplicates()
  WHERE phone_name_group_id = p_phone_name_group_id;

  RETURN jsonb_build_object(
    'phone_name_group_id', v_group.phone_name_group_id,
    'norm_name',  v_group.norm_name, 'norm_phone', v_group.norm_phone,
    ...
    'review_reasons', CASE WHEN v_group.is_owner_cluster_candidate
      THEN jsonb_build_array(jsonb_build_object('rule','R6',
