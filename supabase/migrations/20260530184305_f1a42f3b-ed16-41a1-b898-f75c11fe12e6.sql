-- Add 3 recon/product-intelligence questions to store_questionnaire
ALTER TABLE public.store_questionnaire
  ADD COLUMN IF NOT EXISTS additional_items_wanted text,
  ADD COLUMN IF NOT EXISTS top_selling_items text,
  ADD COLUMN IF NOT EXISTS most_needed_items text;

-- Update apply_field_submission to actually persist store_questionnaire updates
-- into the real columns (the previous version targeted columns that don't exist
-- on this table, so field submissions never landed).
...
BEGIN
  ...
  CASE v_submission.entity_type::text
    ...
    -- ═══ TUBE INVENTORY (legacy signal type) ═══
    ...
    -- ═══ STORE QUESTIONNAIRE (upsert into real columns) ═══
    WHEN 'store_questionnaire' THEN
      IF v_submission.action_type IN ('create', 'update') THEN
        INSERT INTO public.store_questionnaire (
          ...
          security_level,
          sells_flowers,
          interested_cleaning_service,
          additional_items_wanted,
          top_selling_items,
          most_needed_items,
          last_verified_by,
          last_verified_at
    ...
    ELSE
      -- unknown entity types are recorded but not applied
      NULL;
  END CASE;
