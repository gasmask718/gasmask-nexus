CREATE OR REPLACE FUNCTION public._aa_change_summary(p_old jsonb, p_new jsonb, p_fields text[])
RETURNS text
LANGUAGE sql
IMMUTABLE
...
AS $$
  SELECT string_agg(
    f || ': ' ||
    coalesce(nullif(p_old ->> f, ''), '(blank)') || ' -> ' ||
    coalesce(nullif(p_new ->> f, ''), '(blank)'),
    ' | ' ORDER BY f)
  FROM unnest(coalesce(p_fields, '{}'::text[])) AS f
  WHERE f NOT IN ('updated_at','created_at','last_synced_at')
    AND coalesce(p_old ->> f, '') IS DISTINCT FROM coalesce(p_new ->> f, '');
$$;

REVOKE ALL ON FUNCTION public._aa_change_summary(jsonb, jsonb, text[]) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public._aa_change_summary(jsonb, jsonb, text[]) TO authenticated, service_role;

DROP FUNCTION IF EXISTS public.account_activity_feed(text, text, timestamptz, timestamptz, integer, integer);
...
base AS (
  ...
  UNION ALL
  SELECT 'au:'||a.id::text, 'correction', a.record_id, a.acted_at, a.acted_by,
         NULL,
         CASE
           WHEN coalesce(a.new_data ->> 'deleted_at', '') <> '' AND coalesce(a.old_data ->> 'deleted_at','') = '' THEN 'store_removed'
         ...
         END,
         NULL,
         public._aa_change_summary(a.old_data, a.new_data, a.changed_fields)
  FROM audit_log a
  ...
  SELECT 'ic:'||i.id::text, 'invoice', i.store_id, i.created_at,
         ...
         'Invoice #'||coalesce(i.invoice_number,'-')||' created for $'||to_char(coalesce(i.total_amount,0),'FM999999990.00')||
  ...
  SELECT 'ip:'||i.id::text, 'payment', i.store_id, i.paid_at,
         ...
         'Payment $'||to_char(coalesce(i.amount_paid, i.total_amount, 0),'FM999999990.00')||
  ...
  SELECT 'id:'||i.id::text, 'invoice', i.store_id, i.deleted_at, i.deleted_by, NULL,
         ...
         'Invoice #'||coalesce(i.invoice_number,'-')||' ($'||to_char(coalesce(i.total_amount,0),'FM999999990.00')||
