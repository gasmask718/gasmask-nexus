  ...
  WITH base AS (
    SELECT
      ...
      CASE
        ...
        WHEN l.status = 'phone_found' OR l.skip_traced = true THEN 'traced'
        ELSE 'pending'
      END AS skip_status
    ...
    WHERE
      ...
      AND (
        ...
        OR (_skip_status = 'traced'
              AND (l.status = 'phone_found' OR l.skip_traced = true)
        ...
        OR (_skip_status = 'pending'
              ...
              AND COALESCE(l.status,'') NOT IN ('phone_found','skip_trace_failed'))
      )
