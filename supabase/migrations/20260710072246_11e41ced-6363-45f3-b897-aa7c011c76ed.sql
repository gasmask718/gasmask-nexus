...
CREATE OR REPLACE FUNCTION public.sf_lead_summary(
  _states text[] DEFAULT NULL,           -- NULL or empty = all states
  _amount_min numeric DEFAULT NULL,
  _amount_max numeric DEFAULT NULL,
  _skip_status text DEFAULT NULL,        -- 'pending' | 'traced' | 'failed' | NULL
  _status text DEFAULT NULL,             -- exact status match, NULL = all
  _source text DEFAULT NULL,             -- lead_source exact match, NULL = all
  _search text DEFAULT NULL              -- ILIKE across name/county/state/case#
)
RETURNS TABLE (
  total_leads         bigint,
  ...
  avg_surplus         numeric,
  skip_pending_count  bigint,
  skip_traced_count   bigint,
  skip_failed_count   bigint
)
LANGUAGE sql
STABLE
SECURITY INVOKER
  ...
  WITH base AS (
    ...
    WHERE
      ...
      AND (
        ...
        OR (_skip_status = 'traced' AND l.skip_traced = true)
...
GRANT EXECUTE ON FUNCTION public.sf_lead_summary(text[], numeric, numeric, text, text, text, text) TO authenticated, service_role;
