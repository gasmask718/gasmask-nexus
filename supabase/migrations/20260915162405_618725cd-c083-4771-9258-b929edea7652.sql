...
BEGIN
  ...
  -- No conversation context: fall back to phone match, never guessing
  -- between multiple stores. (min() has no uuid overload in Postgres, so the
  -- deterministic pick uses an ordered array_agg instead.)
  SELECT count(DISTINCT sc.id), count(DISTINCT sc.store_id),
         (array_agg(sc.id ORDER BY sc.id))[1],
         (array_agg(sc.store_id ORDER BY sc.store_id))[1]
