-- ============================================================
-- 1. Cache table (mirrors columns of analyze_store_duplicate_groups())
-- ============================================================
CREATE TABLE IF NOT EXISTS public.dynasty_merge_analysis_cache (
...
CREATE INDEX IF NOT EXISTS idx_dynasty_merge_cache_group ON public.dynasty_merge_analysis_cache(duplicate_group_id);
CREATE INDEX IF NOT EXISTS idx_dynasty_merge_cache_winner ON public.dynasty_merge_analysis_cache(is_winner);

ALTER TABLE public.dynasty_merge_analysis_cache ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "admins read merge cache" ON public.dynasty_merge_analysis_cache;
CREATE POLICY "admins read merge cache" ON public.dynasty_merge_analysis_cache
  FOR SELECT TO authenticated
  USING (public.has_role(auth.uid(),'admin') OR public.has_role(auth.uid(),'owner'));

-- ============================================================
-- 2. Meta table (single row tracking last refresh)
-- ============================================================
CREATE TABLE IF NOT EXISTS public.dynasty_merge_analysis_cache_meta (
...
INSERT INTO public.dynasty_merge_analysis_cache_meta (id) VALUES (1) ON CONFLICT (id) DO NOTHING;

ALTER TABLE public.dynasty_merge_analysis_cache_meta ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "admins read merge cache meta" ON public.dynasty_merge_analysis_cache_meta;
CREATE POLICY "admins read merge cache meta" ON public.dynasty_merge_analysis_cache_meta
  FOR SELECT TO authenticated
  USING (public.has_role(auth.uid(),'admin') OR public.has_role(auth.uid(),'owner'));

-- ============================================================
-- 3. Refresh function (admin-only)
-- ============================================================
...
BEGIN
  ...
  SELECT
    a.store_id, a.duplicate_group_id, a.normalized_address, a.group_size, a.store_name, a.raw_address, a.phone,
  ...
  RETURN jsonb_build_object(
    ...
    'duration_seconds', EXTRACT(EPOCH FROM (v_completed - v_started))
  );
END;
$$;

REVOKE ALL ON FUNCTION public.refresh_merge_analysis_cache() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.refresh_merge_analysis_cache() TO authenticated;

-- ============================================================
-- 4. Rewrite preview_store_merge_group() to read from cache
--    Replaces 3 internal calls to analyze_store_duplicate_groups()
--    Same signature, same JSONB output. Read-only.
-- ============================================================
...
BEGIN
  ...
  -- 5. Winner snapshot (CACHE for activity_score)
  ...
  -- 6. Loser loop (CACHE for activity_score)
  ...
  v_estimated_log :=
    1
    + jsonb_array_length(v_losers)
    + jsonb_array_length(v_consolidation)
    + v_tables_affected
    + v_total_dedup_skip
    + v_stores_count + v_master_count
    + 1;
...
-- ============================================================
-- 5. Rewrite analyze_store_duplicate_groups_summary() to read from cache
-- ============================================================
CREATE OR REPLACE FUNCTION public.analyze_store_duplicate_groups_summary()
 RETURNS TABLE(duplicate_group_id integer, normalized_address text, group_size integer, pristine_shell_count bigint, active_record_count bigint, proposed_winner_store_id uuid, proposed_winner_name text, proposed_winner_activity_score bigint, group_classification text, review_priority text)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
