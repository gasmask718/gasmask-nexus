...
-- =============================================
-- Fix: Normalize brand names in sell-through views
-- Maps raw invoice.brand strings to canonical brand IDs
-- Canonical IDs: gasmask, hotmama, scalati, grabba
-- =============================================

-- Helper function to normalize brand strings to canonical IDs
CREATE OR REPLACE FUNCTION public.normalize_brand_name(raw_brand text)
RETURNS text
LANGUAGE sql
IMMUTABLE
...
AS $$
  SELECT CASE
    WHEN LOWER(COALESCE(raw_brand, '')) IN ('gasmask', 'gas mask crm', 'gas mask', 'gasmask bags', 'gasmask tubes') THEN 'gasmask'
    WHEN LOWER(COALESCE(raw_brand, '')) IN ('hotmama', 'hot mama', 'hot-mama') THEN 'hotmama'
    WHEN LOWER(COALESCE(raw_brand, '')) IN ('scalati', 'hot scolatti', 'hotscolatti', 'hot-scolatti', 'hotscolatti-light', 'hotscolatti-dark', 'hot scolatti light', 'hot scolatti dark', 'scolatti') THEN 'scalati'
    WHEN LOWER(COALESCE(raw_brand, '')) IN ('grabba', 'grabba r us', 'grabba-r-us', 'grabbarus') THEN 'grabba'
    ELSE LOWER(COALESCE(raw_brand, ''))
  END
$$;

-- 1) v_store_brand_order_events — now with normalized brand
CREATE OR REPLACE VIEW public.v_store_brand_order_events AS
SELECT
  i.id AS order_id,
  i.store_id,
  public.normalize_brand_name(i.brand) AS brand_name,
...
WHERE i.store_id IS NOT NULL
  ...
  AND public.normalize_brand_name(i.brand) != '';

-- 2) v_store_brand_order_gaps — unchanged logic, benefits from normalized brand
...
-- 3) v_store_brand_sell_through_summary — unchanged logic, benefits from normalized brand
