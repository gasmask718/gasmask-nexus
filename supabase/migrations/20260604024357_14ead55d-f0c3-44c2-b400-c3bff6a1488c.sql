...
DECLARE
  ...
  v_gross numeric; v_qty int; v_price numeric;
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    v_qty   := COALESCE((v_item->>'qty')::int,      (v_item->>'quantity')::int);
    v_price := COALESCE((v_item->>'price_each')::numeric, (v_item->>'unit_price')::numeric);
    INSERT INTO public.marketplace_order_items(order_id, product_id, qty, price_each)
    VALUES (v_order_id, (v_item->>'product_id')::uuid, v_qty, v_price);
  END LOOP;
  ...
  RETURN jsonb_build_object(
    ...
    'affiliate_commission', v_commission, 'ambassador_commission', v_amb_commission,
    'routing', v_routing
  );
END $$;
