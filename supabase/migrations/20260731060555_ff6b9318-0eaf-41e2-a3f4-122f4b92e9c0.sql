CREATE OR REPLACE FUNCTION public.sbo_rebuild_season_splits(_sport text DEFAULT 'mlb', _season text DEFAULT NULL)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  WITH g AS (
    SELECT player_name, player_id, team, game_date, game_id, is_home, stat_line,
           row_number() OVER (PARTITION BY player_name ORDER BY game_date DESC, game_id DESC) AS rn
    FROM public.sbo_player_game_stats
    WHERE sport = _sport
      AND game_date >= (v_season || '-01-01')::date
      AND game_date <= (v_season || '-12-31')::date
  ), kv AS (
    SELECT g.player_name, g.rn, g.is_home, e.key, (e.value #>> '{}')::numeric AS val
    FROM g, LATERAL jsonb_each(g.stat_line) e
    WHERE jsonb_typeof(e.value) = 'number'
  ), per AS (
    SELECT player_name, key,
    ...
    FROM meta m JOIN obj o USING (player_name)
    ON CONFLICT (sport, player_name, season) DO UPDATE SET
...
REVOKE ALL ON FUNCTION public.sbo_rebuild_season_splits(text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.sbo_rebuild_season_splits(text, text) TO service_role;