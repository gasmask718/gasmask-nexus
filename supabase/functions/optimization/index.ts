...
serve(async (req) => {
  ...
  try {
    ...
    if (action === 'calculateRouteInsights') {
      // DEPRECATED 2026-08-04. This writer read the empty `visit_logs` table and
      // padded route_insights with hardcoded service-time/delay estimates.
      // `route-learning` is now the single writer for route_insights and runs
      // nightly on cron against real route_stops / store_visits telemetry.
      return new Response(JSON.stringify({
        error: 'deprecated',
        message: 'route_insights is now written only by the route-learning function (nightly cron).',
      }), {
        status: 410,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
