...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Create event record
    const { data: event } = await supabase.from("ut_pub_events").insert({
      customer_id,
    ...
    // Create booking records per vendor item
    const bookingIds: string[] = [];
    for (const item of selectedItems) {
      const { data: booking } = await supabase.from("ut_bookings").insert({
        customer_id,
    ...
    // Update event build status
    await supabase.from("ut_event_builds").update({ status: "payment_pending" }).eq("id", event_build_id);
