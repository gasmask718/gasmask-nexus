/**
 * AmbassadorProfitTab — Wholesale Box Profit Ledger for Floor 8 Ambassador Profile
 * Read-only mirror of the Ambassador Portal's Profit Dashboard
 * Sources data via admin-scoped RPCs (get_ambassador_profit_dashboard/breakdown)
 */
import { useState, useMemo } from 'react';
import { DollarSign, TrendingUp, Package, BarChart3, ArrowUpRight, ArrowDownRight, AlertTriangle, Download, ShieldCheck, ShieldAlert, Filter } from 'lucide-react';
...
import { useAdminAmbassadorProfitDashboard, useAdminAmbassadorProfitBreakdown } from '@/hooks/useAdminAmbassadorProfit';
...
interface AmbassadorProfitTabProps {
  ambassadorId: string;
}
...
export function AmbassadorProfitTab({ ambassadorId }: AmbassadorProfitTabProps) {
  ...
  const handleExportCSV = () => {
    ...
    exportData({
      filename: `Ambassador_Profit_${timestamp}`,
      format: 'csv',
        ...
        <div className="flex items-start gap-3 p-4 rounded-lg border border-amber-500/30 bg-amber-500/5">
          ...
          <div className="text-sm">
            ...
            <p className="text-muted-foreground">
              {summary?.estimated_row_count} row(s) are missing cost basis or have unverified attribution.
            </p>
          </div>
        </div>
      )}
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {stores.length > 0 && (
                <Select value={storeFilter} onValueChange={setStoreFilter}>
                  <SelectTrigger className="w-[160px] h-8 text-xs"><SelectValue placeholder="Store" /></SelectTrigger>
                  <SelectContent>
              ...
              <Button variant="outline" size="sm" className="h-8 text-xs" onClick={handleExportCSV} disabled={!filteredBreakdown?.length}>
        ...
        <CardContent>
          {(!filteredBreakdown || filteredBreakdown.length === 0) ? (
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="font-medium">No wholesale profit data</p>
              <p className="text-sm">No wholesale box resale activity recorded for this ambassador</p>
            </div>
          ) : (
                ...
                <TableBody>
                  {filteredBreakdown.map((row, idx) => (
                    <TableRow key={idx}>
                      ...
                      <TableCell>
                        {row.sale_channel ? <Badge variant="outline" className="text-xs capitalize">{row.sale_channel}</Badge> : '-'}
                      </TableCell>
                      ...
                      <TableCell className="text-right"><MarginBadge margin={row.margin_pct} /></TableCell>
                      <TableCell className="text-xs text-muted-foreground">
                        {row.sale_month ? format(new Date(row.sale_month), 'MMM yyyy') : '-'}
                      </TableCell>
                      <TableCell><StatusBadge status={row.profit_status} /></TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
