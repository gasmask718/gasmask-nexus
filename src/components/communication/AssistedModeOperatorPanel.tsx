...
import { Phone, Bot, UserCheck, AlertTriangle, Clock, Activity, Sparkles } from "lucide-react";
...
import { AssistedModeCallCard } from "./ai-agent/AssistedModeCallCard";

interface AssistedModeOperatorPanelProps {
  businessId: string;
}

export function AssistedModeOperatorPanel({ businessId }: AssistedModeOperatorPanelProps) {
  const {
    activeSessions,
    recentSessions,
    sessionsLoading,
  ...
  const { data: agentConfig } = useAICallAgentConfig(businessId);
  const { data: trustScore } = useAITrustScore(businessId);

  const isAssistedMode = agentConfig?.mode === 'assisted' || agentConfig?.mode === 'canary' || agentConfig?.mode === 'live';
        ...
        <Alert className="border-primary/50 bg-primary/5">
          ...
          <AlertDescription className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span>AI Call Agent is in</span>
              <Badge variant="outline" className="font-semibold">
                {agentConfig.mode?.toUpperCase() || 'OFF'}
              </Badge>
              <span>mode</span>
              {trustScore && (
        ...
        <div className="flex items-center gap-2 p-3 rounded-lg bg-primary/10 border border-primary/20">
          ...
          {isAssistedMode && (
            <>
              ...
              <span className="text-sm text-primary font-medium">AI Suggestions Enabled</span>
            </>
          )}
        </div>
      )}
            ...
            <Card>
              <CardContent className="py-12 text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  When calls are in progress, they will appear here
                  {isAssistedMode && " with real-time AI suggestions"}.
                </p>
              </CardContent>
            </Card>
          ) : (
