    ...
    <div className={cn('flex flex-wrap gap-2', compact ? 'text-xs' : 'text-sm')}>
      ...
      >
        ...
        {adminOn && state?.reviewed_by_admin_at && (
          <span className="opacity-90 font-normal">· {fmtDate(state.reviewed_by_admin_at)}</span>
        )}
      </Button>
      ...
      >
        ...
        {vaOn && state?.reviewed_by_va_at && (
          <span className="opacity-90 font-normal">· {fmtDate(state.reviewed_by_va_at)}</span>
        )}
      </Button>
    </div>
  );
