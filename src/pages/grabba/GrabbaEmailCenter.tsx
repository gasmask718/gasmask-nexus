...
export default function GrabbaEmailCenter() {
  const { user } = useAuth();
  ...
  const { data: templates } = useQuery({
    queryKey: ['grabba-email-templates'],
    queryFn: async () => {
  ...
  const { data: stores } = useQuery({
    queryKey: ['grabba-stores-for-email', selectedBrand],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('stores')
  ...
  const { data: recentEmails } = useQuery({
    queryKey: ['grabba-recent-emails', selectedBrand],
    queryFn: async () => {
  ...
  const sendEmailMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['grabba-recent-emails'] });
      setSubject('');
      setMessage('');
      setEntityId('');
    },
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    sendEmailMutation.mutate({ entityType, entityId, subject, message });
  };
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Compose and log email communications for Grabba brands
            </p>
          </div>
          <BrandFilterBar
          ...
          <Card className="p-6 bg-card/50 backdrop-blur border-border/50">
            ...
            <div className="space-y-3 max-h-[600px] overflow-y-auto">
              {recentEmails?.map((email) => {
                const brandConfig = email.brand ? GRABBA_BRAND_CONFIG[email.brand as GrabbaBrand] : null;
                return (
              ...
              {(!recentEmails || recentEmails.length === 0) && (
                <p className="text-sm text-muted-foreground text-center py-8">
                  No recent emails for selected brand
                </p>
              )}
            </div>
          </Card>
