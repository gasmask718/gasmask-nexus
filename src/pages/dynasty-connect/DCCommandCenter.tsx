...
import { useDCBusinesses, getDCBusinessIcon } from '@/hooks/useDCBusinesses';
...
export default function DCCommandCenter() {
  ...
  const getBusinessStats = (bizKey: string) => {
    ...
    const bizPhone = phoneNumbers.find((p: any) => p.business === bizKey && p.is_active);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">AI Call Center Command Center · table-driven</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {businesses.map((biz) => {
          const stats = getBusinessStats(biz.business_key);
          const isLive = biz.is_live || !!stats.phone;
          const Icon = getDCBusinessIcon(biz.icon);
          return (
            <Card key={biz.business_key} className="relative overflow-hidden">
              <div className={`absolute top-0 left-0 right-0 h-1 ${biz.color}`} />
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium flex items-center gap-2">
                  <Icon className="h-4 w-4" />
                  {biz.name}
                  {biz.is_internal && <Badge variant="outline" className="text-xs ml-auto">Internal</Badge>}
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="space-y-1 text-sm">
                  <div className="flex justify-between text-muted-foreground"><span>Phone:</span><span className="font-mono text-xs">{stats.phone || biz.phone_default || 'Not set'}</span></div>
                ...
                {biz.agents_label && <div className="text-xs text-muted-foreground">{biz.agents_label}</div>}
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setLauncherBiz({
                        ...
                        name: biz.name,
                        phone: stats.phone || biz.phone_default || undefined,
                      })
                    }
                  >
