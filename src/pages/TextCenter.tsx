...
const TextCenter = () => {
  ...
  const sendTextMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('Text logged successfully (simulation mode)');
      queryClient.invalidateQueries({ queryKey: ['recent-texts'] });
      setMessage('');
      setEntityId('');
    },
    onError: (error) => {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Send and log SMS communications
        </p>
      </div>
        ...
        <Card className="p-6 lg:col-span-2">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" className="w-full" disabled={sendTextMutation.isPending}>
              ...
              {sendTextMutation.isPending ? 'Sending...' : 'Send Text (Simulation)'}
            </Button>

            <p className="text-xs text-muted-foreground text-center">
              ⚠️ This is a simulation. No actual SMS will be sent.
            </p>
          </form>
        </Card>
