...
import { Plus, FileText, Truck, CheckCircle, Clock, Package, ArrowDownToLine } from 'lucide-react';
import { usePurchaseOrders } from '@/services/procurement';
import { format } from 'date-fns';
import { CreatePurchaseOrderModal } from '@/components/warehouse/CreatePurchaseOrderModal';
...
const statusConfig: Record<string, { label: string; variant: 'default' | 'secondary' | 'outline' | 'destructive'; icon: any }> = {
  ...
  ordered: { label: 'Ordered', variant: 'outline', icon: Clock },
  ...
  partially_received: { label: 'Partial', variant: 'outline', icon: ArrowDownToLine },
...
export function WarehousePurchaseOrdersTab() {
  ...
  const [receiveTarget, setReceiveTarget] = useState<{ id: string; poNumber: string } | null>(null);

  const canReceive = (status: string | null) =>
    ['draft', 'ordered', 'placed', 'paid', 'in_transit', 'partially_received', 'delivered'].includes(status || '');
  ...
  return (
    <>
            ...
            <Table>
              ...
              <TableBody>
                {purchaseOrders.map((po) => {
                  ...
                  const poNumber = (po as any).po_number || `PO-${po.id.slice(0, 8).toUpperCase()}`;
                  return (
                    <TableRow key={po.id}>
                      <TableCell className="font-mono font-medium text-sm">{poNumber}</TableCell>
                      <TableCell>{(po as any).supplier_name || po.supplier?.name || 'Unknown'}</TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <div className="flex gap-1">
                          {canReceive(po.status) && (
                            <Button size="sm" variant="outline" onClick={() => setReceiveTarget({ id: po.id, poNumber })}>
      ...
      <CreatePurchaseOrderModal open={createOpen} onClose={() => setCreateOpen(false)} />
      {receiveTarget && (
        <ReceiveStockModal
          open={!!receiveTarget}
          onClose={() => setReceiveTarget(null)}
          poId={receiveTarget.id}
          poNumber={receiveTarget.poNumber}
        />
      )}
    </>
  );
