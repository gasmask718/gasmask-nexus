...
import { Car, CheckCircle, Wrench, Layers, Plus } from 'lucide-react';
        ...
        {vehicles.length === 0 ? (
          ...
          return (
            <TableRow key={v.id} className="border-white/5">
              <TableCell>
                <div>
                  <p className="text-white font-medium text-sm">{[v.year, v.make, v.model].filter(Boolean).join(' ') || 'Unknown'}</p>
              ...
              <TableCell>
                {editingRate === v.id ? (
                  ...
                  <span className="text-[#C9A84C] font-bold cursor-pointer hover:underline" onClick={() => { setEditingRate(v.id); setRateValue(String(v.base_rate || '')); }}>${Number(v.base_rate || 0).toLocaleString()}/hr</span>
                )}
              </TableCell>
    ...
    <div className="space-y-6">
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent className="bg-[#111111] border-l border-[#C9A84C]/10 text-white w-[500px] sm:max-w-[500px] overflow-y-auto">
          {selectedVehicle && (
            <>
              <SheetHeader><SheetTitle className="text-white">{[selectedVehicle.year, selectedVehicle.make, selectedVehicle.model].filter(Boolean).join(' ')}</SheetTitle></SheetHeader>
              <div className="mt-4 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div><p className="text-white/40">Base Rate</p><p className="text-[#C9A84C] font-bold">${Number(selectedVehicle.base_rate || 0).toLocaleString()}/hr</p></div>
