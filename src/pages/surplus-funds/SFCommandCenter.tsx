...
import { Users, Phone, Briefcase, DollarSign, TrendingUp, Search, Upload, Scale, UserPlus } from 'lucide-react';
...
const PIPELINE_STAGES = [
  ...
  { key: 'interested', label: 'Interested', color: 'bg-teal-500' },
  { key: 'agreement_signed', label: 'Agreement', color: 'bg-amber-500' },
  { key: 'case_filed', label: 'Case Filed', color: 'bg-orange-500' },
...
export default function SFCommandCenter() {
  ...
  const { data: cases = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('surplus_funds_cases').select('status, surplus_amount, amount_received, our_expected_fee');
      return data ?? [];
    },
  });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-amber-500">Surplus Funds OS</h1>
        <p className="text-muted-foreground">Recovery pipeline — leads to closing</p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card className="border-amber-500/20">
          ...
          <CardContent className="flex flex-wrap gap-3">
            <Button onClick={() => navigate('/surplus-funds/leads')} className="bg-amber-600 hover:bg-amber-700"><Upload className="h-4 w-4 mr-2" />Upload Leads CSV</Button>
            <Button variant="outline" onClick={() => navigate('/surplus-funds/leads')}><UserPlus className="h-4 w-4 mr-2" />Add Lead</Button>
